...
RNG::RNG(uint32_t seed, uint32_t index) {

	curand_init(WangHash(seed + index), 0, 0, &rngState);
}

RT_HOSTDEVICE float RNG::RandomFloat(){

	return curand_uniform(&rngState);
}

RT_HOSTDEVICE uint32_t RNG::RandomUInt() {
...
RT_HOSTDEVICE uint32_t RNG::RandomUInt(uint32_t bound) {
